...
export interface HighlightConfig<T = any> {
   ...
   *   [highlightedRow] здесь — начальный seed: применяется при переходе значения из null в
   *   не-null (а не только на первом изменении), дальше состоянием владеет таблица. То есть
   *   сброс входа в null и новое значение засеют его повторно.
